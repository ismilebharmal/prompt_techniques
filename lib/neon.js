...
export async function createProject(projectData) {
  ...
  const result = await sql`
    INSERT INTO projects (title, description, short_description, image_url, github_url, live_url, technologies, category, featured, order_index, created_at, updated_at) 
    VALUES (${title}, ${description}, ${shortDescription}, ${imageUrl}, ${githubUrl}, ${liveUrl}, ${JSON.stringify(technologies)}, ${category}, ${featured || false}, ${orderIndex || 0}, NOW(), NOW())
    RETURNING id
  `
...
export async function createSlide(slideData) {
  ...
  const { title, description, imageUrl, category, orderIndex } = slideData
  
  const result = await sql`
    INSERT INTO slides (title, description, image_url, category, order_index, active, created_at, updated_at) 
    VALUES (${title}, ${description}, ${imageUrl}, ${category}, ${orderIndex || 0}, true, NOW(), NOW())
    RETURNING id
  `
...
export async function updateSlide(id, slideData) {
  ...
  const { title, description, imageUrl, category, orderIndex, active } = slideData

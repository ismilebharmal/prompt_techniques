...
export async function createHeroSlide(title, description, imageId, displayOrder = 0) {
  if (!sql) {
  ...
  try {
    const result = await sql`
      INSERT INTO hero_slides (title, description, image_id, display_order, is_active)
      VALUES (${title}, ${description}, ${imageId}, ${displayOrder}, true)
      RETURNING *
    `
    return result[0]
  } catch (error) {
...
export async function updateHeroSlide(id, title, description, imageId, displayOrder, isActive) {
  if (!sql) {
    console.log('Neon not configured, returning null')
    return null
  }

  ...
  try {
    ...
    result.forEach(row => {
      ...
      if (row.image_filename) {
        ...
        slidesMap.get(row.id).images.push(imageData)
        
        // Set cover image fields for admin dashboard compatibility
        if (row.is_cover) {
          slidesMap.get(row.id).imageId = row.image_id
          slidesMap.get(row.id).imageUrl = `/api/images/${row.image_id}`
        }
      }
    })
...
export async function createSkill(skillData) {
  ...
  const { name, level, color, category = 'Technologies', display_order = 0 } = skillData

  const result = await sql`
    INSERT INTO skills (name, level, color, category, display_order)
    VALUES (${name}, ${level}, ${color}, ${category}, ${display_order})
    RETURNING *
  `
...
export async function updateSkill(id, skillData) {
  ...
  const { name, level, color, category, display_order, active } = skillData
...
export async function reorderSkills(skillOrders) {
  ...
  for (const { id, display_order } of skillOrders) {
    await sql`
      UPDATE skills 
...
export async function createFeaturedProject(projectData) {
  try {
    const {
      title,
      description,
...
export async function updateFeaturedProject(id, projectData) {
  try {
    const {
      title,
      description,

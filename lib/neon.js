...
export async function createHeroSlide(title, description, imageId, displayOrder = 0, imagePosition = 'center', imageFit = 'cover', imageOpacity = 100, textPosition = 'bottom-left', customCss = '') {
  if (!sql) {
  ...
  try {
    const result = await sql`
      INSERT INTO hero_slides (title, description, image_id, display_order, is_active, image_position, image_fit, image_opacity, text_position, custom_css)
      VALUES (${title}, ${description}, ${imageId}, ${displayOrder}, true, ${imagePosition}, ${imageFit}, ${imageOpacity}, ${textPosition}, ${customCss})
      RETURNING *
    `
    return result[0]
  } catch (error) {
...
export async function updateHeroSlide(id, title, description, imageId, displayOrder, isActive, imagePosition = 'center', imageFit = 'cover', imageOpacity = 100, textPosition = 'bottom-left', customCss = '') {
  if (!sql) {
    console.log('Neon not configured, returning null')
    return null
  }

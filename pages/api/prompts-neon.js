import { getPrompts, createPrompt } from '../../lib/neon'
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      // If Neon returns empty data, use fallback
      if (prompts.length === 0) {
        console.log('Neon returned empty data, using fallback prompts')
    ...
    // Fallback to hardcoded prompts when Neon fails or is empty
    if (req.method === 'GET') {
      console.log('Using fallback prompts - Neon not configured or empty')
      const fallbackPrompts = [
        {
          _id: "68c28bf185b75ada59b55d57",
          title: "Email Rewrite — Professional",
